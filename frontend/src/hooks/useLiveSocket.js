...
// Tách toàn bộ logic kết nối Socket.io + state realtime ra khỏi UI,
// để DashboardPage chỉ còn lo hiển thị, không lo kết nối.
    ...
    useEffect(() => {
        ...
        socket.on('MEMBER_JOIN', (data) => {
            ...
            const item = {
                ...
                user: data.nickname || data.username || 'Người xem',
                timestamp: data.receivedAt || data.createTime || Date.now(),
            };
